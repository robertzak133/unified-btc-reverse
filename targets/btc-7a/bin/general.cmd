/* Command File for Gcc Loader */
/*    Created automatically by Ghidra.UnifiedCreateLdScript.java */
/*    2025-04-12 06:30:38 */

  /* Symbol Definitions */
   btc_strcpy = 0x8002d36c;
   g_left_button_enable = 0x803be8ae;
   g_ColdItemData = 0x803d19c0;
   init_directory_suffix_file_prefix = 0x801a8024;
   get_current_video_duration_in_seconds = 0x80118b04;
   set_fsm_state_absolute = 0x80122df4;
   hal_set_rtc = 0x80055378;
   update_timelapse_sunrise = 0x801128b8;
   get_fine_grained_time = 0x800033a0;
   g_battery_type_menu = 0x803b3bc8;
   handleRestoreDefault_menu = 0x8013336c;
   get_cold_item_timelapse_frequency = 0x8010e444;
   g_image_resolution_lookup_table = 0x80353aa0;
   g_photo_sensor_value = 0x803be91c;
   g_ReferenceColdItemData = 0x803d1b08;
   g_menu_root = 0x803bea94;
   g_HceTaskMenuMultiItem_fsm_function_array = 0x8035527c;
   get_capture_timer_rtc_time = 0x8010fa88;
   get_g_evt_0x52510507_counter = 0x8010e948;
   positive_diff = 0x80055ed8;
   checkForSDCard = 0x80114aec;
   menu_redraw_items = 0x80131384;
   set_fast_cap_mount_active_p = 0x80115390;
   draw_rectangle_wrapper = 0x8011ecf8;
   tty_printf_battery_stats = 0x800e3134;
   TaskTimeLapseFSM_task13 = 0x80129bac;
   TaskTimeLapseFSM_task12 = 0x80129bd8;
   set_rtc_extra_operation_mode = 0x8005573c;
   still_low_battery_display_function = 0x801220f8;
   g_video_quality_menu = 0x803b3714;
   service_button_event = 0x8010dba4;
   g_photo_delay_menu = 0x803b3768;
   ui_cursor_key_pressed_p = 0x80112150;
   get_max_hour = 0x80131ab8;
   IRLedOff = 0x80111874;
   get_next_state_from_menu_enter = 0x801318e8;
   DigiPIRSpi_Write = 0x8012d720;
   get_power_supply_mode = 0x801134fc;
   get_temperatureForC = 0x8010f004;
   get_g_menu_temp_hour = 0x80131aa4;
   get_SDCardState = 0x80114a94;
   local_sprintf = 0x800209e0;
   g_dcfapi_loaded_p = 0x804a4314;
   get_am_pm_max_value = 0x80131aec;
   get_battery_voltage_x100 = 0x80113554;
   initialize_g_mode_change_counter = 0x80117958;
   fsm_getCurrentState = 0x80122dd0;
   update_timelapse_rise_set_times = 0x80113130;
   g_smart_ir_video_menu = 0x803b3f64;
   HceIQ_CheckNightMode = 0x80055c28;
   HceTask_ToNextNChar = 0x801359d0;
   get_camera_photo_resolution = 0x8010ff9c;
   HceCommon_RestoreDefaultColdItem = 0x80055f3c;
   set_rtc_extra_byte_range = 0x8005c5d0;
   fatVolLabSet = 0x8002c7a4;
   get_rtc_extra_operation_mode = 0x80055764;
   get_cold_item_multi_shot_encoding = 0x8010e414;
   vfsClose = 0x8002a660;
   g_operation_mode_menu = 0x803b3554;
   g_video_length_menu = 0x803b3650;
   set_g_info_strip_enabled_p = 0x801153a0;
   get_battery_percent_from_voltage = 0x80113718;
   debug_print_string = 0x80009c3c;
   g_firmware_upgrade_menu = 0x803b4140;
   get_battery_percent = 0x801137e4;
   encoded_timelapse_frequency_to_seconds = 0x80110158;
   memoryAllocate = 0x800125bc;
   get_g_sd_card_valid_p = 0x8010e978;
   g_event_descriptor = 0x803ef0d8;
   get_am_pm_current_value = 0x80131adc;
   get_tod_in_timelapse_region = 0x80112b38;
   HceStampDrawLogo = 0x8011e064;
   get_within_operating_hours_p = 0x8010df1c;
   g_last_timelapse_time_in_ms = 0x8037bbb4;
   read_photo_sensor_value = 0x80055c00;
   set_g_some_sd_status_p = 0x80114bd8;
   menu_get_next_menu_selection = 0x801315e8;
   set_g_sd_card_mounted_p = 0x80114a9c;
   btc_strlen = 0x80012bd8;
   g_delete_all_menu = 0x803b3ea0;
   schedule_alarm_global_callback = 0x8005c9e0;
   get_cold_item_camera_name = 0x8010e47c;
   cold_item_led_power_blur_reduction_p = 0x8010e4d4;
   setStillCapDone = 0x80114000;
   reset_capture_timer = 0x8010df40;
   temperature_sensor_getReading = 0x8010f01c;
   g_new_check_night_mode_p = 0x803bd2f4;
   getCurrentMenuCollectionAndSize = 0x80130808;
   get_min_hour = 0x80131ab0;
   photo_sensor_hysteresis = 0x80055714;
   tty_printf = 0x80003c9c;
   check_post_printf_state_set_sio_params = 0x80057bf0;
   register_low_battery_display_function = 0x801271f4;
   get_power_switch_on_p = 0x80055be8;
   HceTaskStillFSM_valid_p = 0x80127208;
   TaskTimeLapseFSM_task14_end = 0x801288c0;
   posix_fileinfo = 0x8002fd04;
   serviceSetCurrentMode_event = 0x801122f4;
   handleSetTimeMenu = 0x801360c4;
   memcpy = 0x8000a420;
   TaskTimeLapseFSM_task3_ae_set = 0x801298cc;
   get_min_day = 0x80131a30;
   get_min_minute = 0x80131acc;
   get_g_menu_temp_minute = 0x80131ac0;
   set_g_temperature_forc = 0x8010eff4;
   get_multi_shot_photo_dimensions = 0x8010ffcc;
   btc_fwrite = 0x8005ce1c;
   event_loop_iterator = 0x80111bec;
   set_cold_item_timelapse_new_file_p = 0x8010ee38;
   g_sd_management_menu = 0x803b3fb8;
   log_printf = 0x800036c0;
   g_right_button_enable = 0x803be8af;
   btc_fread = 0x8005c12c;
   get_max_year = 0x80131a00;
   function_with_syscall_zero = 0x80007f40;
   get_device_csr_bit = 0x80009e60;
   HceCommon_SetCaptureImag = 0x8010e74c;
   draw_video_scroll_bar = 0x80136910;
   TaskTimeLapseFSM_task8_CopyJPGFromRAM = 0x801293d0;
   get_rtc_time_or_alarm = 0x8005c660;
   set_sd_card_state_w_init = 0x80114a60;
   get_g_temp_day_number = 0x80131a24;
   get_cold_item_capture_timer_p = 0x8010fa5c;
   get_g_menu_temp_year = 0x801319ec;
   fsm_iterate_all_in_g_fsm_descriptor_list = 0x80122d54;
   fsize = 0x8005c064;
   get_max_day = 0x80131a38;
   set_g_evt_0x5251050a_counter = 0x8010e950;
   set_pre_printf_state = 0x80057bd4;
   get_min_month = 0x80131a14;
   Write_LEDOn = 0x80115f00;
   getCameraConfigStructPtr = 0x80122eb8;
   seekToSpecifiedFileLocation = 0x8005be58;
   HceIRCut_SetIRCutOpen = 0x801114cc;
   g_pir_range_menu = 0x803b3b74;
   get_g_fast_cap_mount_active_p = 0x80115388;
   startHceTaskStill_FSM = 0x8012722c;
   g_capture_timer_menu = 0x803b40ec;
   free = 0x800126f0;
   set_g_sd_card_state_valid_p = 0x8010e970;
   digi_pir_spi_serial_out = 0x8012d6c0;
   get_g_sd_card_mounted_p = 0x80114aa4;
   setSensorDigitalEffectPhoto = 0x801107d0;
   host_task_init = 0x801122c4;
   some_sort_of_delay = 0x8012d674;
   TaskTimeLapseFSM_task11_openTLfile = 0x80128908;
   get_cold_item_photo_resolution = 0x8010e380;
   serviceEvent_0x5151000 = 0x8011232c;
   set_g_cold_item_battery_type = 0x8010e564;
   get_cold_item_tod_last_photo_in_seconds = 0x8010edcc;
   get_cold_item_temperature_unit_celsius_p = 0x8010e42c;
   get_g_cold_item_led_power = 0x80055ef8;
   TaskTimeLapseFSM_task10_WaitMountSD = 0x80129c24;
   g_image_data_strip_menu = 0x803b3acc;
   g_motion_test_menu = 0x803b3b20;
   update_timelapse_sunset = 0x80112948;
   handleCommitUpdates_menu = 0x80134d30;
   get_current_operating_time_ms = 0x80003488;
   get_matching_event_descriptor = 0x80111b38;
   service_event_0x5851000 = 0x80112374;
   event_descriptor_init = 0x80111b1c;
   setBatteryCalibConfig = 0x80113e6c;
   g_restore_default_menu = 0x803b3c70;
   get_min_year = 0x801319f8;
   app_init_directory_suffix_file_prefix = 0x80114a40;
   g_set_date_time_menu = 0x803b3538;
   get_short_rtc_time = 0x8010ec1c;
   get_next_state_from_menu_mode = 0x8013183c;
   get_g_evt_0x5251050a_counter = 0x8010e958;
   g_set_time_buffer = 0x803b3394;
   g_camera_name_menu = 0x803b3ab0;
   g_mode_button_enable = 0x803be8b1;
   g_language_menu = 0x803b400c;
   btc_init_directory_suffix_file_prefix = 0x80118aec;
   thread_sleep = 0x80026588;
   vfs_unmount_wrapper2 = 0x80114ba8;
   g_photo_detector_hysteresis = 0x803be91a;
   btc_fclose = 0x8005c340;
   get_rtc_extra_byte_range = 0x8005c618;
   g_up_button_enable = 0x803be8ac;
   setDigitalEffectswitchdataD_table = 0x80355ad0;
   get_am_pm_min_value = 0x80131ae4;
   TaskTimeLapseFSM_task9 = 0x801293a0;
   TaskTimeLapseFSM_task7 = 0x801294b4;
   menu_draw_selected_item = 0x8013167c;
   g_cold_item_signature_valid_p = 0x803be90c;
   TaskTimeLapseFSM_task6 = 0x8012963c;
   TaskTimeLapseFSM_task5 = 0x80129740;
   IRCutThreadCreate = 0x801114dc;
   g_temp_am_pm_p = 0x803bd340;
   get_g_menu_temp_month = 0x80131a08;
   TaskTimeLapseFSM_task2 = 0x80129eec;
   TaskTimeLapseFSM_task1 = 0x80129a50;
   TaskTimeLapseFSM_task0 = 0x80129ac0;
   Volt_Calib_Bat = 0x80113ca0;
   execute_if_not_null = 0x8010df8c;
   g_multi_shot_mode_menu = 0x803b389c;
   draw_sst_string_on_display = 0x8011eaec;
   power_on_IR_LED = 0x801116ac;
   g_temp_unit_menu = 0x803b3a5c;
   set_exif_time_of_capture = 0x80112434;
   g_active_dcfapi_functions = 0x803bc97c;
   g_night_mode_p = 0x803be919;
   set_fsm_state_relative = 0x80122e44;
   get_max_month = 0x80131a1c;
   set_g_evt_0x52510507_counter = 0x8010e940;
   btc_fopen = 0x8005bde0;
   update_time_field = 0x801315b8;
   HceIRCut_SetIRCutClosed = 0x801114d4;
   get_g_cold_item_battery_type = 0x8010e570;
   get_cold_item_operation_mode = 0x8010e334;
   set_g_evt_0x52510507_state_initialized_p = 0x8010e938;
   set_cold_item_current_tod_in_seconds = 0x8010ee20;
   get_max_minute = 0x80131ad4;
   get_current_date_time_short = 0x8010eba8;
   store_pressure_trend = 0x8010ef0c;
   g_camera_setup_menu_item_array = 0x803b41b0;
   fatVolLabSet_wrapper = 0x802fbdc4;
   get_g_test_mode = 0x8010f254;
   draw_set_time_screen = 0x80135234;
   get_photo_size_factor = 0x80110128;
   HceTaskBoot2Cap_Task0 = 0x801221e4;
   Write_LEDOff = 0x80115f94;
   set_some_sd_global = 0x80114bc8;
   fsm_getNextState = 0x80122e20;
   g_down_button_enable = 0x803be8ad;
   get_cold_item_timelapse_period = 0x8010e45c;
   g_trigger_speed_menu = 0x803b3c1c;
   g_set_date_time_menu_state = 0x803ef25c;
   check_remaining_sd_capacity = 0x801150e8;
   setSensorDigitalEffectVideo = 0x80110b84;
   g_enter_button_enable = 0x803be8b0;

   _gp = 0x803c5100;

/* Memory */
MEMORY {
  entry0_ram(WXAI) : ORIGIN = 0x80057190, LENGTH = 0x328
  entry1_ram(WXAI) : ORIGIN = 0x8006c5a0, LENGTH = 0x3b4
  entry2_ram(WXAI) : ORIGIN = 0x8017630c, LENGTH = 0x6198
}
/* Sections */
SECTIONS {
       entry0_section 0x80057190 : {
                entry0.o
                ir-flash-menu-early.o
      } >entry0_ram
       entry1_section 0x8006c5a0 : {
                entry1.o
                aperture.o
      } >entry1_ram
       entry2_section 0x8017630c : {
                entry2.o
                capture-timer.o
                custom-ribbon.o
                custom-info-strip.o
                reduce-sd-clock.o
                volume-file-naming.o
                extend-event-SD-card.o
                custom-settings.o
                rtc-formats.o
                dslr-trigger.o
                menus.o
                timelapse.o
                aperture-menu.o
                ir-flash-menu.o
                custom-set-date-time.o
                native-quality.o
                white-flash.o
      } >entry2_ram
}
