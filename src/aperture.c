...
void apt_set_rtc_extra_operation_mode(uint operation_mode) {
...
void apt_get_night_mode_threshold_min_max(uint *min, uint *max, 
  ...
  *min = g_apt_nightmode_threshold_lookup_table[(uint) encoded_aperture].min;
  *max = g_apt_nightmode_threshold_lookup_table[(uint) encoded_aperture].max;
  return;
}
